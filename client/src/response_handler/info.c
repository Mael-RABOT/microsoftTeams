...
void info_handler(client_t *client, codes_t code, char *response)
{
    ...
    if (!data)
        return;
    if (code == USER_PRINT)
        return (void)client->logger->client_print_user(
            data[0], data[1], data[2][0] - '0');
    if (code == TEAM_PRINT)
        return (void)client->logger->client_print_team(
            data[0], data[1], data[2]);
    if (code == CHANNEL_PRINT)
        return (void)client->logger->client_print_channel(
            data[0], data[1], data[2]);
    if (code == THREAD_PRINT) {
        timestamp = atol(data[2]);
        return (void)client->logger->client_print_thread(
